...
import com.instructure.androidfoosball.views.WinTableKingGameDialog
import kotlinx.android.synthetic.tablet.activity_table_king_game.*
import org.jetbrains.anko.sdk21.listeners.onClick
import org.jetbrains.anko.textColor
import java.util.*

class TableKingGameActivity : AppCompatActivity() {
    ...
    private val mGame by lazy { App.realm.where(TableKingGame::class.java).equalTo("id", mGameId).findFirst()!! }
    private val mDatabase: DatabaseReference = FirebaseDatabase.getInstance().reference
    private val mTable = Table.getSelectedTable()
    private val mHistory = Stack<TableKingGame>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_table_king_game)
    ...
    private val goalReceiver: GoalReceiver = GoalReceiver { side ->
        if (mGame.isGameComplete) return@GoalReceiver
        mHistory += App.realm.copyFromRealm(mGame)
        val (willWinRound, willEndGame) = mGame.getGoalOutcome(side)
        if (!willEndGame) {
            if (willWinRound) {
                val announcement = "${Sfx.DING} ${Sfx.ROTATE_DING} ${mGame.getNextSwappedPlayers().joinToString("and") { it.name }} - swap now"
                mCommentator.announce(announcement, true)
            } else {
                val (team1, team2) = mGame.currentTeams
                val (scoringTeam, opposingTeam) = if (side.isSide1) team1 to team2 else team2 to team1
                val (scoringName, opposingName) = with(mTable) {
                    if (side.isSide1) sideOneName to sideTwoName else sideTwoName to sideOneName
                }
                mCommentator.announceGoal(scoringTeam.teamName.elseIfBlank(scoringName),
                        opposingTeam.teamName.elseIfBlank(opposingName), scoringTeam.points + 1, opposingTeam.points)
            }
        }
    ...
    private fun updateState() {
        ...
        val sideOneServing = mGame.getServingSide() == TableSide.SIDE_1
        ...
        if (mGame.isGameComplete) {
            ...
            WinTableKingGameDialog(this, mGame, { this.undoGoal() }, { this.endGame() }).show()
        }
    }
    ...
    private fun updateGameStatusBusy() {
        mDatabase.child("tables")
                .child(mTable.id)
                .child("currentGame").setValue("TABLE_KING")
    }
