...
class AssignmentViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    companion object {
        const val HOLDER_RES_ID = R.layout.adapter_assignment
    }
    ...
    fun bind(context: Context, assignment: Assignment, courseColor: Int, callback: (Assignment) -> Unit) = with(itemView){
        ...
        assignmentLayout.contentDescription = assignment.name + " " + dueDate.text + " " + ungradedCount.text + " " + if(assignment.published) context.getString(R.string.published) else context.getString(R.string.not_published)
    }
}
