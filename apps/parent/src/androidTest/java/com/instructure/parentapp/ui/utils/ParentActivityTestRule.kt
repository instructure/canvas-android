...
class ParentActivityTestRule<T : Activity>(activityClass: Class<T>) : InstructureActivityTestRule<T>(activityClass) {

    override fun performReset(context: Context) {
        PandaAppResetter.reset(context)
        PreviousUsersUtils.clear(context)
        PandaAppResetter.getEditor(context, ApplicationManager.PREF_NAME).apply { clear(); apply() }
        PandaAppResetter.getEditor(context, Const.CANVAS_PARENT_SP).apply { clear(); apply() }
    }
