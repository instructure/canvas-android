...
public class OpenPollExpandableListFragment extends PaginatedExpandableListFragment<String, PollSession> {
    ...
    @Override
    public boolean onRowClick(PollSession item) {
        //we haven't gotten the poll data yet, don't let it go any farther
        if(!pollMap.containsKey(item.getPollId())) {
            return true;
        }

        startActivityForResult(StudentPollActivity.createIntent(
                getActivity(), pollMap.get(item.getPollId()), item, item.getHasSubmitted()), Constants.SUBMIT_POLL_REQUEST);
        return true;
    }
    ...
    public void loadFirstPage() {
        PollsManager.getOpenSessions(pollSessionCallback, true);
    }
            ...
            public void onResponse(@NonNull Response<PollSessionResponse> response, @NonNull LinkHeaders linkHeaders, @NonNull ApiType type) {
                ...
                if(pollSessions != null) {
                    for (PollSession pollSession : pollSessions) {
                        ...
                        if(pollSession.getHasSubmitted()) {
                            addItem(getString(R.string.answeredPolls), pollSession);
                        }
                        else {
                            addItem(getString(R.string.unansweredPolls), pollSession);
                        }
                    }
                    expandAllGroups();
                }
