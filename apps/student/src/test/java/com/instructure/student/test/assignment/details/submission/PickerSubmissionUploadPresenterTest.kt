...
class PickerSubmissionUploadPresenterTest : Assert() {
    ...
    fun `returns file view states and submit visible when there are files`() {
        ...
        val fileViewStates = listOf(
            PickerListItemViewState(
                0,
                R.drawable.ic_utils_attachment,
                baseFile.name,
                "${baseFile.size} B"
            ),
            PickerListItemViewState(
    ...
    fun `returns correct file size`() {
        ...
        val fileViewStates = listOf(
            PickerListItemViewState(0, R.drawable.ic_utils_attachment, smallFile.name, "1 KB"),
            PickerListItemViewState(1, R.drawable.ic_utils_attachment, mediumFile.name, "1 MB"),
            PickerListItemViewState(2, R.drawable.ic_utils_attachment, largeFile.name, "1 GB")
        )
        val expectedState = PickerSubmissionUploadViewState.FileList(
            baseVisibilities.copy(submit = true),
            fileViewStates
