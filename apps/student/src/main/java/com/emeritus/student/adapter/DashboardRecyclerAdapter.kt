...
import com.emeritus.student.holders.CourseHeaderViewHolder
...
import com.emeritus.student.holders.CourseViewHolder
import com.emeritus.student.holders.GroupHeaderViewHolder
import com.emeritus.student.holders.GroupViewHolder
import com.emeritus.student.interfaces.CourseAdapterToFragmentCallback
...
class DashboardRecyclerAdapter(
    ...
    override fun onBindChildHolder(holder: RecyclerView.ViewHolder, header: ItemType, item: Any) {
        when {
            holder is CourseViewHolder && item is Course -> holder.bind(
                item,
                mAdapterToFragmentCallback
            )
            holder is GroupViewHolder && item is Group -> holder.bind(
                item,
                mCourseMap,
                mAdapterToFragmentCallback
            )
    ...
    override fun createGroupCallback(): GroupSortedList.GroupComparatorCallback<ItemType> {
        return object : GroupSortedList.GroupComparatorCallback<ItemType> {
            ...
            override fun areContentsTheSame(oldGroup: ItemType, newGroup: ItemType) =
                oldGroup == newGroup
    ...
    override fun loadData() {
        ...
        mApiCalls = tryWeave {
            ...
            val dashboardCards =
                awaitApi<List<DashboardCard>> { CourseManager.getDashboardCourses(isRefresh, it) }
            ...
            // Filter groups
            val allActiveGroups =
                groups.filter { group -> group.isActive(mCourseMap[group.courseId]) }

            val isAnyFavoritePresent =
                visibleCourses.any { it.isFavorite } || allActiveGroups.any { it.isFavorite }
            val visibleGroups =
                if (isAnyFavoritePresent) allActiveGroups.filter { it.isFavorite } else allActiveGroups
    ...
    private fun hasValidCourseForEnrollment(enrollment: Enrollment): Boolean {
        return mCourseMap[enrollment.courseId]?.let { course ->
            course.isValidTerm() && !course.accessRestrictedByDate && isEnrollmentBeforeEndDateOrNotRestricted(
                course
            )
        } ?: false
    }
    ...
    private fun isEnrollmentBeforeEndDateOrNotRestricted(course: Course): Boolean {
        val isBeforeEndDate = course.endAt?.let {
            ...
            val endDate =
                OffsetDateTime.parse(it).withOffsetSameInstant(OffsetDateTime.now().offset)
