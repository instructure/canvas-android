...
object ShortcutUtils {
    ...
    private fun generateLayeredBitmap(context: Context, color: Int): Bitmap? {
        try {
            val background =
                BitmapUtilities.getBitmapFromDrawable(context, R.mipmap.ic_shortcut_background)
            val foreground = ColorUtils.colorIt(color,
                BitmapUtilities.getBitmapFromDrawable(context, R.drawable.ic_navigation_bookmarks)
            )
            ...
            layers[1] = BitmapDrawable(context.resources,
                BitmapUtilities.addPaddingToBitmap(foreground, padding)
            )
