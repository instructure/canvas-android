...
class CalendarEventFragment : ParentFragment() {
    ...
    private fun initViews() {
        with (calendarEventWebViewWrapper.webView) {
            ...
            canvasEmbeddedWebViewCallback = object : CanvasWebView.CanvasEmbeddedWebViewCallback {
                override fun launchInternalWebViewFragment(url: String) = RouteMatcher.route(requireActivity(),
    ...
    companion object {
        ...
        fun makeRoute(canvasContext: CanvasContext, scheduleItem: ScheduleItem): Route =
                Route(null, CalendarEventFragment::class.java, canvasContext, canvasContext.makeBundle { putParcelable(
                    SCHEDULE_ITEM, scheduleItem) })

        fun makeRoute(canvasContext: CanvasContext, scheduleItemId: Long): Route =
                Route(null, CalendarEventFragment::class.java, canvasContext, canvasContext.makeBundle { putLong(
        ...
        private fun validRoute(route: Route): Boolean = route.canvasContext != null &&
                (route.arguments.containsKey(SCHEDULE_ITEM) || route.arguments.containsKey(
                    SCHEDULE_ITEM_ID
                ))
    }
}
