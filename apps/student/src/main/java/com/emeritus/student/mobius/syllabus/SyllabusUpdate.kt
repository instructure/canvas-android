...
class SyllabusUpdate : UpdateInit<SyllabusModel, SyllabusEvent, SyllabusEffect>() {
    override fun performInit(model: SyllabusModel): First<SyllabusModel, SyllabusEffect> {
        return First.first(model.copy(isLoading = true), setOf<SyllabusEffect>(
            SyllabusEffect.LoadData(
                model.courseId,
                false
            )
        ))
    ...
    override fun update(model: SyllabusModel, event: SyllabusEvent): Next<SyllabusModel, SyllabusEffect> {
        return when (event) {
            SyllabusEvent.PullToRefresh -> Next.next(model.copy(isLoading = true), setOf<SyllabusEffect>(
                SyllabusEffect.LoadData(model.courseId, true)
            ))
            ...
            is SyllabusEvent.SyllabusItemClicked -> {
                ...
                Next.dispatch(setOf(
                    when {
                        item.assignment != null -> SyllabusEffect.ShowAssignmentView(
                            item.assignment!!,
                            model.course!!.dataOrThrow
                        )
                        else -> SyllabusEffect.ShowScheduleItemView(
                            item,
                            model.course!!.dataOrThrow
                        )
                    }
