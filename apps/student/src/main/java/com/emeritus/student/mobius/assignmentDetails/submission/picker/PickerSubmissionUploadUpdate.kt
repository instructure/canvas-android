...
class PickerSubmissionUploadUpdate :
    ...
    override fun performInit(model: PickerSubmissionUploadModel): First<PickerSubmissionUploadModel, PickerSubmissionUploadEffect> {
        return if (model.mediaFileUri == null) {
            ...
            First.first(model.copy(isLoadingFile = true), setOf(
    ...
    override fun update(
        ...
        PickerSubmissionUploadEvent.SubmitClicked -> Next.dispatch(setOf(
            PickerSubmissionUploadEffect.HandleSubmit(model)
        ))
        PickerSubmissionUploadEvent.CameraClicked -> Next.dispatch(setOf(
            PickerSubmissionUploadEffect.LaunchCamera
        ))
        PickerSubmissionUploadEvent.GalleryClicked -> Next.dispatch(setOf(
            PickerSubmissionUploadEffect.LaunchGallery
        ))
        PickerSubmissionUploadEvent.SelectFileClicked -> Next.dispatch(setOf(
            PickerSubmissionUploadEffect.LaunchSelectFile
        ))
        PickerSubmissionUploadEvent.DocumentScanningClicked -> Next.dispatch(setOf(
