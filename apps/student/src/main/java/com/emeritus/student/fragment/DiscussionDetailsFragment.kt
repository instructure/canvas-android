...
import com.emeritus.student.events.DiscussionTopicHeaderEvent
import com.emeritus.student.events.DiscussionUpdatedEvent
import com.emeritus.student.events.ModuleUpdatedEvent
...
class DiscussionDetailsFragment : ParentFragment(), Bookmarkable {
    ...
    fun onBackStackChangedEvent(event: OnBackStackChangedEvent) {
        event.get { clazz ->
            if (clazz?.isAssignableFrom(DiscussionDetailsFragment::class.java) == true || clazz?.isAssignableFrom(
                    CourseModuleProgressionFragment::class.java) == true) {
                discussionRepliesWebViewWrapper.webView.onResume()
                discussionTopicHeaderWebViewWrapper.webView.onResume()
            } else {
                discussionRepliesWebViewWrapper.webView.onPause()
