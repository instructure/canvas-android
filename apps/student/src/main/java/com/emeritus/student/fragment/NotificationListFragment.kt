...
import com.emeritus.student.activity.ParentActivity
import com.emeritus.student.adapter.NotificationListRecyclerAdapter
import com.emeritus.student.features.assignmentdetails.AssignmentDetailsFragment
import com.emeritus.student.interfaces.NotificationAdapterToFragmentCallback
    ...
    companion object {
        fun addFragmentForStreamItem(streamItem: StreamItem, context: Context, fromWidget: Boolean) {
                ...
                if (conversation != null) {
                    ...
                    if (conversation.isDeleted) {
                        ...
                        RouteMatcher.route(context,
                            InboxConversationFragment.makeRoute(conversation, null)
                        )
                    }
                }
                return
            ...
            val route: Route? = when (streamItem.getStreamItemType()) {
                ...
                COLLABORATION -> UnsupportedTabFragment.makeRoute(
                    canvasContext,
                    Tab.COLLABORATIONS_ID
                )
                CONFERENCE -> ConferenceListFragment.makeRoute(canvasContext)
                else -> UnsupportedFeatureFragment.makeRoute(
                    canvasContext,
                    featureName = streamItem.type,
                    url = streamItem.url ?: streamItem.htmlUrl
                )
            }
