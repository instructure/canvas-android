...
import com.emeritus.student.adapter.PeopleListRecyclerAdapter
        ...
        override fun onRowClicked(user: User, position: Int, isOpenDetail: Boolean) {
            if (canvasContext.isCourse) {
                RouteMatcher.route(requireContext(),
                    PeopleDetailsFragment.makeRoute(user.id, canvasContext)
                )
            } else {
                RouteMatcher.route(requireContext(),
                    PeopleDetailsFragment.makeRoute(user, canvasContext)
                )
            }
        }
