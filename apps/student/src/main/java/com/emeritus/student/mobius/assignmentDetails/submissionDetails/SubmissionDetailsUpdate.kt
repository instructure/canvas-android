...
class SubmissionDetailsUpdate : UpdateInit<SubmissionDetailsModel, SubmissionDetailsEvent, SubmissionDetailsEffect>() {
    override fun performInit(model: SubmissionDetailsModel): First<SubmissionDetailsModel, SubmissionDetailsEffect> {
        return First.first(
            ...
            setOf(
                SubmissionDetailsEffect.LoadData(
                    model.canvasContext.id,
                    model.assignmentId,
                    model.isObserver
                )
            )
    ...
    override fun update(model: SubmissionDetailsModel, event: SubmissionDetailsEvent): Next<SubmissionDetailsModel, SubmissionDetailsEffect> {
        return when (event) {
            ...
            is SubmissionDetailsEvent.VideoRecordingReplayClicked -> {
                if(event.file != null) {
                    Next.dispatch<SubmissionDetailsModel, SubmissionDetailsEffect>(setOf(
                        SubmissionDetailsEffect.ShowVideoRecordingPlayback(event.file)
                    ))
                } else {
                    Next.dispatch<SubmissionDetailsModel, SubmissionDetailsEffect>(setOf(
            ...
            is SubmissionDetailsEvent.SendMediaCommentClicked -> {
                if(event.file != null) {
                    Next.dispatch<SubmissionDetailsModel, SubmissionDetailsEffect>(setOf(
                        SubmissionDetailsEffect.UploadMediaComment(event.file)
                    ))
                } else {
                    Next.dispatch<SubmissionDetailsModel, SubmissionDetailsEffect>(setOf(
    ...
    private fun getSubmissionContentType(
        ...
        return when {
            ...
            submission.workflowState != "submitted" && AssignmentUtils2.getAssignmentState(assignment, submission) in listOf(AssignmentUtils2.ASSIGNMENT_STATE_MISSING, AssignmentUtils2.ASSIGNMENT_STATE_GRADED_MISSING) -> SubmissionDetailsContentType.NoSubmissionContent(
                canvasContext,
                assignment!!,
                isStudioEnabled!!,
                quiz
                ...
                // Text submission
                Assignment.SubmissionType.ONLINE_TEXT_ENTRY -> SubmissionDetailsContentType.TextContent(
                    submission.body ?: ""
                )
                ...
                // URL Submission
                Assignment.SubmissionType.ONLINE_URL -> SubmissionDetailsContentType.UrlContent(
                    submission.url!!,
                ...
                Assignment.SubmissionType.DISCUSSION_TOPIC -> SubmissionDetailsContentType.DiscussionContent(
    ...
    private fun getAttachmentContent(attachment: Attachment): SubmissionDetailsContentType {
        var type = attachment.contentType ?: return SubmissionDetailsContentType.OtherAttachmentContent(
            attachment
        )
        if (type == "*/*") {
