...
class ToDoListFragment : ParentFragment() {
    ...
    private var adapterToFragmentCallback: NotificationAdapterToFragmentCallback<ToDo> = object :
        NotificationAdapterToFragmentCallback<ToDo> {
        ...
        when {
            ...
            toDo?.scheduleItem != null -> // It's a Calendar event from the Upcoming API.
                RouteMatcher.route(requireContext(),
                    CalendarEventFragment.makeRoute(toDo.canvasContext!!, toDo.scheduleItem!!)
                )
            toDo?.quiz != null -> // It's a Quiz let's launch the quiz details fragment
                RouteMatcher.route(requireContext(),
                    BasicQuizViewFragment.makeRoute(
                        toDo.canvasContext!!,
                        toDo.quiz!!,
                        toDo.quiz!!.url!!
                    )
