 ...
 */
package com.emeritus.student.mobius.assignmentDetails.submission.url
...
class UrlSubmissionUploadUpdate : UpdateInit<UrlSubmissionUploadModel, UrlSubmissionUploadEvent, UrlSubmissionUploadEffect>() {
    override fun performInit(model: UrlSubmissionUploadModel): First<UrlSubmissionUploadModel, UrlSubmissionUploadEffect> {
        return First.first(model, setOf<UrlSubmissionUploadEffect>(
            UrlSubmissionUploadEffect.InitializeUrl(
                model.initialUrl
            )
        ))
    }
    ...
    override fun update(model: UrlSubmissionUploadModel, event: UrlSubmissionUploadEvent): Next<UrlSubmissionUploadModel, UrlSubmissionUploadEffect> {
        ...
        return when(event) {
            is UrlSubmissionUploadEvent.UrlChanged -> {
                ...
                Next.next(model.copy(isSubmittable = true, urlError = MalformedUrlError.NONE), setOf(
                    UrlSubmissionUploadEffect.ShowUrlPreview(url)
                ))
            }
            is UrlSubmissionUploadEvent.SubmitClicked -> {
                Next.dispatch(Effects.effects((UrlSubmissionUploadEffect.SubmitUrl(
                    event.url,
                    model.course,
                    model.assignmentId,
                    model.assignmentName
                )) as UrlSubmissionUploadEffect
                ))
            }
        }
    }
