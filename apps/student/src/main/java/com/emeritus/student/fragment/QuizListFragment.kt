...
import com.emeritus.student.adapter.QuizListRecyclerAdapter
...
class QuizListFragment : ParentFragment(), Bookmarkable {
    ...
    private var adapterToFragmentCallback: AdapterToFragmentCallback<Quiz> = object :
        AdapterToFragmentCallback<Quiz> {
        ...
        if (navigation != null) {
            ...
            if (RouteMatcher.canRouteInternally(requireActivity(), quiz.htmlUrl!!, ApiPrefs.domain, false)) {
                ...
                RouteMatcher.route(requireContext(),
                    BasicQuizViewFragment.makeRoute(canvasContext, quiz, quiz.url!!)
                )
            }
        }
    }
