...
import com.emeritus.student.fragment.NotificationListFragment
import com.emeritus.student.service.StudentPageViewService
import com.emeritus.student.util.StudentPrefs
import kotlinx.coroutines.Job
import retrofit2.Call
import retrofit2.Response

abstract class CallbackActivity : ParentActivity(), OnUnreadCountInvalidated,
    NotificationListFragment.OnNotificationCountInvalidated {
    ...
    private fun loadInitialData() {
        loadInitialDataJob = tryWeave {
            ...
            val termsOfService =
                awaitApi<TermsOfService> { UserManager.getTermsOfService(it, true) }
            ApiPrefs.canGeneratePairingCode =
                termsOfService.selfRegistrationType == SelfRegistration.ALL
            ...
            if (!ApiPrefs.showElementaryView) {
                if (ColorKeeper.previouslySynced) {
                    ...
                    ColorKeeper.addToCache(awaitApi<CanvasColor> {
                        UserManager.getColors(
                            it,
                            true
                        )
                    })
            ...
            if (ThemePrefs.isThemeApplied) {
                ...
                ThemePrefs.applyCanvasTheme(
                    awaitApi { ThemeManager.getTheme(it, true) },
            ...
            val launchDefinitions = awaitApi<List<LaunchDefinition>?> {
                LaunchDefinitionsManager.getLaunchDefinitions(
                    it,
                    false
                )
            }
            launchDefinitions?.let {
                val definitions =
                    launchDefinitions.filter { it.domain == LaunchDefinition._STUDIO_DOMAIN || it.domain == LaunchDefinition._GAUGE_DOMAIN }
    ...
    private suspend fun getUnreadMessageCount() {
        val unreadCount = awaitApi<UnreadConversationCount> {
            UnreadCountManager.getUnreadConversationCount(
                it,
                true
            )
        }
    ...
    private fun getUnreadNotificationCount() {
        UnreadCountManager.getUnreadNotificationCount(object :
            StatusCallback<List<UnreadNotificationCount>>() {
            override fun onResponse(
                data: Call<List<UnreadNotificationCount>>,
                response: Response<List<UnreadNotificationCount>>
            ) {
                updateNotificationCount(response.body()?.sumOf { it.unreadCount.orDefault() }
                    .orDefault())
            }
        }, true)
    }
