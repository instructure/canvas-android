...
import com.emeritus.student.activity.NotificationWidgetRouter
import com.emeritus.student.util.StringUtilities
...
class NotificationViewWidgetService : BaseRemoteViewsService(), Serializable {
    ...
    private inner class NotificationsRowFactory(private val intent: Intent) : CanvasWidgetRowFactory<StreamItem>() {
        ...
        override fun setViewData(streamItem: StreamItem, row: RemoteViews) {
            val appWidgetId = getAppWidgetId(intent)
            ...
            row.setTextColor(R.id.title, getWidgetTextColor(appWidgetId, applicationContext))
            ...
            if (streamItem.canvasContext != null && streamItem.canvasContext?.type != CanvasContext.Type.USER) {
                ...
                val color = if(streamItem.canvasContext != null) getCanvasContextTextColor(appWidgetId, streamItem.canvasContext)
                            else getWidgetTextColor(appWidgetId, ContextKeeper.appContext)
            ...
            if (!shouldHideDetails(appWidgetId)) {
                if (streamItem.getMessage(ContextKeeper.appContext) != null) {
                    ...
                    row.setTextColor(R.id.message, getWidgetSecondaryTextColor(appWidgetId, applicationContext))
                } else {
            ...
            row.setTextColor(R.id.course_and_date, getWidgetSecondaryTextColor(appWidgetId, applicationContext))
