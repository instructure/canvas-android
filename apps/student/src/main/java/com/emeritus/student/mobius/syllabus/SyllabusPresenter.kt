 ...
 *
 */
package com.emeritus.student.mobius.syllabus
...
import com.emeritus.student.util.toDueAtString
import java.util.*
...
object SyllabusPresenter : Presenter<SyllabusModel, SyllabusViewState> {
    ...
    private fun getIcon(event: ScheduleItem): Int {
        if (event.assignment?.isLocked == true || event.assignment?.lockExplanation?.takeIf {
            ...
            return com.emeritus.student.R.drawable.ic_lock_lined
        }

        return event.assignment?.let {
            getAssignmentIcon(it)
        } ?: com.emeritus.student.R.drawable.ic_calendar
    }

    private fun getAssignmentIcon(assignment: Assignment) = when {
        assignment.getSubmissionTypes().contains(Assignment.SubmissionType.ONLINE_QUIZ) -> com.emeritus.student.R.drawable.ic_quiz
        assignment.getSubmissionTypes().contains(Assignment.SubmissionType.DISCUSSION_TOPIC) -> com.emeritus.student.R.drawable.ic_discussion
        else -> com.emeritus.student.R.drawable.ic_assignment
    }
}
