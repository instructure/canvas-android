...
import com.emeritus.student.activity.InterwebsToApplication

class TodoWidgetProvider : CanvasWidgetProvider() {
    ...
    override fun getRefreshIntent(appWidgetManager: AppWidgetManager): Intent {
        return WidgetUpdater.getTodoWidgetUpdateIntent(appWidgetManager)
    }
    ...
    override fun setWidgetDependentViews(context: Context, remoteViews: RemoteViews, appWidgetId: Int, appWidgetManager: AppWidgetManager, textColor: Int) {
        remoteViews.setRemoteAdapter(R.id.contentList,
            TodoViewWidgetService.createIntent(context, appWidgetId)
        )
        remoteViews.setTextViewText(R.id.widget_title, context.getString(R.string.todoWidgetTitle))

        remoteViews.setInt(R.id.widget_root, "setBackgroundResource",
            BaseRemoteViewsService.getWidgetBackgroundResourceId(appWidgetId)
        )
    ...
    companion object {
        private const val TODO_REFRESH_ID = 1
        private const val SIMPLE_NAME = "Todo Widget"
    }
}
