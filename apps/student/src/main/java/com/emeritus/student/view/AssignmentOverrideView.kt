...
class AssignmentOverrideView @JvmOverloads constructor(
    ...
    fun setupOverride(
        ...
        // Maybe be needed in the future for UI testing candroid
//        if (BuildConfig.IS_TESTING) removeOverride.contentDescription = "remove_override_button_$index"
    ...
    fun validateInput(): Boolean {
        ...
        with(mDueDateGroup.coreDates) {
            ...
            dueDate?.let {
                if (unlockDate?.after(it) ?: false) {
                    ...
                    fromDateTextInput.error = context.getString(R.string.utils_unlockAfterDueDateError)
                }
                ...
                if (lockDate?.before(it) ?: false) {
                    ...
                    toDateTextInput.error = context.getString(R.string.utils_lockBeforeDueDateError)
                }
            }
            unlockDate?.let {
                if (lockDate?.before(it) ?: false) {
                    ...
                    saveError = true
                    toDateTextInput.isErrorEnabled = true
                    toDateTextInput.error = context.getString(R.string.utils_lockAfterUnlockError)
                }
            }
        }
        ...
        if (!mDueDateGroup.hasOverrideAssignees && !mDueDateGroup.isEveryone) {
            assignToTextInput.error = context.getString(R.string.utils_assigneeBlankError)
            saveError = true
        }
