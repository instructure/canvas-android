    ...
    private fun presentLoadedState(
        ...
        val (submittedLabelRes, submittedColorRes, submittedIconRes) = if (assignment.isSubmitted) {
            ...
            if (assignment.submission?.missing == true || isMissingFromDueDate) {
                ...
                Triple(R.string.missingSubmissionLabel, R.color.submissionStatusColorMissing, R.drawable.ic_unsubmitted)
            } else {
                Triple(R.string.notSubmitted, R.color.defaultTextGray, R.drawable.ic_unsubmitted)
            }
        }
