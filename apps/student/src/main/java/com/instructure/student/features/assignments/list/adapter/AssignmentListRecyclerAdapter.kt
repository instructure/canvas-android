...
abstract class AssignmentListRecyclerAdapter(
    ...
    private fun loadAssignmentsData(course: Course) {
        ...
        if (currentGradingPeriod != null && currentGradingPeriod!!.title != null
            && currentGradingPeriod!!.title == context.getString(R.string.assignmentsListDisplayGradingPeriod)) {
            loadAssignment()
            return
        }
    ...
    override fun onBindChildHolder(
        ...
        (holder as AssignmentViewHolder).bind(context, assignment, canvasContext.color, adapterToAssignmentsCallback, restrictQuantitativeData, gradingSchemes)
    }
