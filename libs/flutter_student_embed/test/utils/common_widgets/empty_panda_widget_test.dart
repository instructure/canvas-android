...
void main() {
  final svgPath = 'assets/svg/panda-not-supported.svg';
  final title = 'title';
  ...
  testWidgetsWithAccessibilityChecks('shows an svg and a title', (tester) async {
    ...
    expect(find.byType(SvgPicture), findsOneWidget);
    expect(find.byType(SizedBox), findsWidgets); // The spacing between the svg and the title
    expect(find.byType(Text), findsOneWidget);
    expect(find.text(title), findsOneWidget);
  });
  ...
  testWidgetsWithAccessibilityChecks('shows an svg and a subtitle', (tester) async {
    ...
    expect(find.byType(SvgPicture), findsOneWidget);
    expect(find.byType(SizedBox), findsWidgets); // The spacing between the svg and the subtitle
    expect(find.byType(Text), findsOneWidget);
    expect(find.text(subtitle), findsOneWidget);
  });
  ...
  testWidgetsWithAccessibilityChecks('shows an svg, a title, and a subtitle', (tester) async {
    ...
    expect(find.byType(SvgPicture), findsOneWidget);
    expect(find.byType(SizedBox), findsWidgets); // Between the svg and title, as well as title and subtitle
    expect(find.byType(Text), findsNWidgets(2));
    expect(find.text(title), findsOneWidget);
    expect(find.text(subtitle), findsOneWidget);
  });
