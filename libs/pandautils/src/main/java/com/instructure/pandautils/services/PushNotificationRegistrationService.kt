    ...
    override fun onStartJob(job: JobParameters): Boolean {
        ...
        val runnable = Runnable {
            ...
            try {
                ...
                token = instanceID.getToken(job.extras?.getString(Const.PROJECT_ID) ?: "", GoogleCloudMessaging.INSTANCE_ID_SCOPE, null)
                responseBody = CommunicationChannelsManager.addNewPushCommunicationChannelSynchronous(token, object : StatusCallback<ResponseBody>() {})
                ...
                Crashlytics.logException(YUNoWorkPushNotifsException("PushNotificationRegistrationService : onStartJob() - Error registering push notifications.\n" +
                        ...
                        "InstanceId: $instanceID\n\t" +
                            "Id: ${instanceID?.id}\n\t" +
                            "Creation Time: ${instanceID?.creationTime}\n\t" +
                            "Subtype: ${instanceID?.subtype}\n" +
                        "Token: $token\n" +
