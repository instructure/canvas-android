...
class CourseSync(
    ...
    private suspend fun fetchCalendarEvents(courseId: Long): List<ScheduleItem> {
        val restParams = RestParams(usePerPageQueryParam = true, isForceReadFromNetwork = true, shouldRefreshToken = false)
    ...
    private suspend fun fetchPages(courseId: Long) {
        fetchTab(courseId, Tab.PAGES_ID) {
            val params = RestParams(usePerPageQueryParam = true, isForceReadFromNetwork = true, shouldRefreshToken = false)
    ...
    private suspend fun fetchHomePage(courseId: Long) {
        try {
            val frontPage = pageApi.getFrontPage(
                ...
                courseId,
                RestParams(isForceReadFromNetwork = true, shouldRefreshToken = false)
            ).dataOrNull
            if (frontPage != null) {
    ...
    private suspend fun getConferencesForContext(
        ...
        val params = RestParams(usePerPageQueryParam = true, isForceReadFromNetwork = forceNetwork, shouldRefreshToken = false)
