...
class CreateUpdateToDoViewModelTest {
    ...
    private val plannerItem = PlannerItem(
        ...
        plannable = Plannable(
            ...
            dueAt = null,
            assignmentId = null,
            todoDate = LocalDate.of(2024, 2, 22).atTime(12, 0).toApiString(),
            startAt = null,
            endAt = null,
            details = "Description"
        ),
    ...
    fun `Course list gets fetched`() {
        ...
        Assert.assertEquals(courses, viewModel.uiState.value.courses)
        Assert.assertEquals(courses.last(), viewModel.uiState.value.selectedCourse)
    }
    ...
    fun `Saves new ToDo when creating`() = runTest {
        ...
        viewModel.handleAction(CreateUpdateToDoAction.UpdateCourse(Course(1)))
    ...
    private fun createViewModel() {
        viewModel = CreateUpdateToDoViewModel(context, savedStateHandle, repository)
    }
}
