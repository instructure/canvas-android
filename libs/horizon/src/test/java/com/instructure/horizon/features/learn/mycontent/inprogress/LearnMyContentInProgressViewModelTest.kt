...
class LearnMyContentInProgressViewModelTest {
    ...
    @Test
    fun `onFiltersChanged triggers load with IN_PROGRESS and NOT_STARTED status`() = runTest {
        val viewModel = getViewModel()
    ...
    fun `onFiltersChanged with NameAscending sort passes NAME_A_Z sort option`() = runTest {
        ...
        coVerify {
            repository.getLearnItems(
                cursor = null,
                searchQuery = null,
                sortBy = CollectionItemSortOption.NAME_A_Z,
                status = any(),
                itemTypes = null,
                forceNetwork = false,
            )
    ...
    fun `Refresh success clears error and updates content`() = runTest {
        coEvery { repository.getLearnItems(any(), any(), any(), any(), any(), false) } throws Exception("Error")
        val viewModel = getViewModel()
        viewModel.onFiltersChanged("", LearnLearningLibrarySortOption.MostRecent, LearnLearningLibraryTypeFilter.All)

        coEvery { repository.getLearnItems(any(), any(), any(), any(), any(), true) } returns LearnItemsResponse(
    ...
    fun `loadMore error shows snackbar and clears isMoreLoading`() = runTest {
        coEvery { repository.getLearnItems(null, any(), any(), any(), any(), any()) } returns LearnItemsResponse(
            items = listOf(createTestProgramItem()),
            pageInfo = LearningLibraryPageInfo("cursor1", null, true, false, 2, null)
        )
        val viewModel = getViewModel()
        viewModel.onFiltersChanged("", LearnLearningLibrarySortOption.MostRecent, LearnLearningLibraryTypeFilter.All)
        coEvery { repository.getLearnItems("cursor1", any(), any(), any(), any(), any()) } throws Exception("Network error")
    ...
    @Test
    fun `Empty search query passes null searchQuery to repository`() = runTest {
        val viewModel = getViewModel()
    ...
    private fun getViewModel() = LearnMyContentInProgressViewModel(resources, repository)
